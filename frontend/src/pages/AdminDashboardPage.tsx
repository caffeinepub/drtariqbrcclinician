...
import { useIsCallerAdmin, useGetAllAppointments, useGetAllPatientDetails, useGetPaymentDetails, useGetEmailSettings, useUpdateAppointmentStatus } from '../hooks/useQueries';
...
import { Calendar, History, Users, Settings, CreditCard, CheckCircle, Clock, AlertCircle, ClipboardList, ArrowUpDown, Search } from 'lucide-react';
import { Status, ConsultationType, Appointment } from '../backend';
...
export default function AdminDashboardPage() {
  ...
  const { data: patients, isLoading: patientsLoading } = useGetAllPatientDetails();
  ...
  const pendingCount = appointments?.filter(a => a.status === Status.pending).length || 0;
  const completedCount = appointments?.filter(a => a.status === Status.completed).length || 0;
  ...
  const handleStatusChange = async (appointmentId: bigint, newStatus: Status) => {
  ...
  const dashboardCards = [
    ...
    },
    {
      ...
      stat: completedCount > 0 ? `${completedCount} completed` : 'No history yet',
      loading: appointmentsLoading,
    },
    {
    ...
    },
    {
      ...
      stat: emailLoading ? 'Loading...' : `Email notifications ${emailStatus.toLowerCase()}`,
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      ...
      <div className="mb-8">
        ...
        <p className="text-lg text-gray-600">Manage your clinic operations from one central location</p>
      </div>
      ...
      <div className="mb-8 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{appointmentsLoading ? '...' : pendingCount}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{appointmentsLoading ? '...' : completedCount}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{patientsLoading ? '...' : totalPatients}</div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">{emailLoading ? '...' : emailStatus}</div>
            <p className="text-xs text-muted-foreground">Notifications {emailLoading ? 'loading' : emailStatus.toLowerCase()}</p>
          </CardContent>
        </Card>
      </div>
      ...
      <Card className="mb-8">
        <CardHeader>
          ...
          <CardDescription>Complete list of all patient bookings with detailed information</CardDescription>
        </CardHeader>
        <CardContent>
          ...
          <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
            <div className="flex flex-col gap-4 md:flex-row md:items-center">
              ...
              <div className="flex items-center gap-2">
                <label className="text-sm font-medium" style={{ color: '#FF0000' }}>
                  Status:
                </label>
              ...
              <div className="flex items-center gap-2">
                ...
                >
                  <ArrowUpDown className="h-4 w-4" style={{ color: '#FF0000' }} />
                </Button>
              </div>
            </div>
          ...
          {!appointmentsLoading && appointments && (
            <div className="mb-4">
              <p className="text-sm text-gray-600">
                Showing <span className="font-semibold" style={{ color: '#FF0000' }}>{filteredAndSortedAppointments.length}</span> of{' '}
          ...
          {appointmentsLoading ? (
            ...
            <div className="py-8 text-center">
              ...
              <p className="text-lg font-medium text-gray-600">No appointments yet</p>
              <p className="text-sm text-gray-500">New appointments will appear here</p>
            </div>
          ) : filteredAndSortedAppointments.length === 0 ? (
            <div className="py-8 text-center">
              <Search className="mx-auto mb-3 h-12 w-12 text-gray-400" />
              <p className="text-lg font-medium text-gray-600">No appointments match your filters</p>
              ...
              <Table>
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead style={{ color: '#FF0000' }}>Contact Info</TableHead>
                    <TableHead>
                      <button
                    ...
                    <TableHead style={{ color: '#FF0000' }}>Consultation Type</TableHead>
                    <TableHead style={{ color: '#FF0000' }}>Health Concerns</TableHead>
                    <TableHead>
                      <button
                    ...
                    <TableHead style={{ color: '#FF0000' }}>Date Submitted</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredAndSortedAppointments.map((appointment) => (
                    <TableRow key={appointment.id.toString()} className={appointment.isNew ? 'bg-yellow-50' : ''}>
                      <TableCell className="font-medium">
                        {appointment.patientName}
                        {appointment.isNew && (
                          <span className="ml-2 inline-block rounded-full bg-red-100 px-2 py-0.5 text-xs font-semibold" style={{ color: '#FF0000' }}>
                            New
                          </span>
                        )}
                      </TableCell>
                      <TableCell className="max-w-[200px] truncate">{appointment.contactInfo}</TableCell>
                      ...
                      <TableCell className="max-w-[250px]">
                        <div className="line-clamp-2" title={appointment.healthConcerns}>
                      ...
                      <TableCell>
                        <Select
                          ...
                          onValueChange={(value) => handleStatusChange(appointment.id, value as Status)}
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Current supported payment methods and UPI details</CardDescription>
        </CardHeader>
        <CardContent>
          {paymentLoading ? (
            ...
            <div className="space-y-4">
              <div>
                <h4 className="mb-2 font-semibold text-gray-900">Payment Methods:</h4>
              ...
              <div className="grid gap-2 md:grid-cols-2">
                <div>
                  <p className="text-sm font-medium text-gray-600">Payment Number:</p>
                  <p className="text-base font-semibold text-gray-900">{paymentDetails.methods[0]?.number}</p>
                </div>
                <div>
                  <p className="text-sm font-medium text-gray-600">UPI ID:</p>
                  <p className="text-base font-semibold text-gray-900">{paymentDetails.methods[0]?.upiId}</p>
                </div>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-600">Email:</p>
                <p className="text-base font-semibold text-gray-900">{paymentDetails.email}</p>
              </div>
              <div>
                <p className="text-sm font-medium text-gray-600">Currency:</p>
                <p className="text-base font-semibold text-gray-900">{paymentDetails.currency} (₹)</p>
              </div>
            </div>
          ) : (
