...
import { useGetAllAppointments, useUpdateAppointmentStatus, useIsCallerAdmin, useMarkAppointmentsAsViewed } from '../hooks/useQueries';
import { Card, CardContent, CardHeader, CardTitle, CardDescription } from '@/components/ui/card';
import { Table, TableBody, TableCell, TableHead, TableHeader, TableRow } from '@/components/ui/table';
import { Select, SelectContent, SelectItem, SelectTrigger, SelectValue } from '@/components/ui/select';
import { Badge } from '@/components/ui/badge';
import { Button } from '@/components/ui/button';
import { Shield, Calendar, Clock, User, Phone, FileText, AlertCircle, ArrowUpDown, Sparkles, RefreshCw } from 'lucide-react';
import { toast } from 'sonner';
import { Status, ConsultationType } from '../backend';

type SortField = 'dateSubmitted' | 'preferredDateTime' | 'patientName' | 'status';
...
export default function AdminAppointmentsPage() {
  ...
  const shouldFetchAppointments = isAuthenticated && !adminCheckLoading && isAdmin === true;
  
  const { data: appointments, isLoading: appointmentsLoading, refetch: refetchAppointments, isFetched } = useGetAllAppointments();
  ...
  }, [shouldFetchAppointments, appointments]);
  ...
  const sortedAppointments = useMemo(() => {
    ...
    console.log('Appointment statuses:', appointments.map(a => ({ id: a.id.toString(), status: a.status, isNew: a.isNew })));
    ...
    // Sort each group
    const sortFn = (a: typeof appointments[0], b: typeof appointments[0]) => {
      let comparison = 0;
      ...
      <div className="container mx-auto px-4 py-16">
        <Card className="mx-auto max-w-2xl border-amber-200 bg-amber-50">
          <CardContent className="flex items-start gap-4 p-8">
            ...
            <div>
              <h2 className="mb-2 text-xl font-semibold text-amber-900">Authentication Required</h2>
          ...
          <CardContent className="flex items-start gap-4 p-8">
            ...
            <div>
              ...
              <p className="text-red-800">
                You do not have permission to access this page. Only administrators can manage appointments.
              </p>
            </div>
          </CardContent>
        </Card>
  ...
  const pendingCount = sortedAppointments.filter((a) => a.status === Status.pending).length;
  const confirmedCount = sortedAppointments.filter((a) => a.status === Status.confirmed).length;
  const completedCount = sortedAppointments.filter((a) => a.status === Status.completed).length;
  const cancelledCount = sortedAppointments.filter((a) => a.status === Status.cancelled).length;
    ...
    <div className="container mx-auto px-4 py-16">
      ...
      <div className="mb-8">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="mb-2 text-4xl font-bold text-emerald-800">Appointments Management</h1>
          ...
          >
            <RefreshCw className={`h-4 w-4 ${appointmentsLoading ? 'animate-spin' : ''}`} style={{ color: '#FF0000' }} />
            Refresh
          </Button>
        </div>
      </div>
        ...
        <CardContent>
          {appointmentsLoading ? (
            ...
            <div className="py-12 text-center">
              ...
              <p className="text-sm text-gray-500">Appointments will appear here once patients book them</p>
            </div>
          ) : (
                ...
                <TableHeader>
                  <TableRow>
                    ...
                    <TableHead>
                      ...
                      >
                        <User className="h-4 w-4" style={{ color: '#FF0000' }} />
                        Patient Name
                        <ArrowUpDown className="h-3 w-3" style={{ color: '#FF0000' }} />
                      </Button>
                    </TableHead>
                    <TableHead>
                    ...
                    <TableHead>
                      <div className="flex items-center gap-1 font-semibold">
                        <FileText className="h-4 w-4" style={{ color: '#FF0000' }} />
                        Type
                      </div>
                    </TableHead>
                    <TableHead>
                    ...
                    <TableHead>
                      <Button
                        ...
                        className="flex items-center gap-1 font-semibold"
                      >
                        <Calendar className="h-4 w-4" style={{ color: '#FF0000' }} />
                        Submitted
                        <ArrowUpDown className="h-3 w-3" style={{ color: '#FF0000' }} />
                      </Button>
                    </TableHead>
                ...
                <TableBody>
                  {sortedAppointments.map((appointment) => (
                    ...
                    >
                      ...
                      <TableCell className={appointment.isNew ? 'font-bold' : 'font-medium'}>
                        {appointment.patientName}
                      </TableCell>
                      <TableCell className="max-w-[200px] truncate text-sm">{appointment.contactInfo}</TableCell>
                      <TableCell className="text-sm">{appointment.preferredDateTime}</TableCell>
                      <TableCell>
                      ...
                      <TableCell className="max-w-[250px]">
                        <p className="line-clamp-2 text-sm text-gray-600">{appointment.healthConcerns}</p>
                      </TableCell>
                      <TableCell>
                        <Select
